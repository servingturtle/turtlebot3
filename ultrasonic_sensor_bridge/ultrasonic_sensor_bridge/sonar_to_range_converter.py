...
class SonarToRangeConverter(Node):
    def __init__(self):
        ...
        self.get_logger().info("turtlebot3_msgs/SensorState의 sonar 데이터를 3개의 sensor_msgs/Range로 변환합니다.")

    def sensor_state_callback(self, msg: SensorState):
        """SensorState 메시지 콜백"""
        current_time = self.get_clock().now()
        
        # sonar 데이터가 있으면 사용, 없으면 시뮬레이션 값 사용
        sonar_distance = msg.sonar if msg.sonar > 0 else self.front_distance
        
        # 왼쪽 센서 데이터 (sonar 값에 약간의 변형 적용)
        left_msg = self.create_range_message(
            current_time, 
            "ultrasonic_left", 
            sonar_distance * 0.8  # sonar 값의 80%
        )
        self.ul_pub_left.publish(left_msg)
        
        # 앞쪽 센서 데이터 (sonar 값 그대로 사용)
        ...
        # 오른쪽 센서 데이터 (sonar 값에 약간의 변형 적용)
        right_msg = self.create_range_message(
            current_time, 
            "ultrasonic_right", 
            sonar_distance * 0.9  # sonar 값의 90%
        )
        self.ul_pub_right.publish(right_msg)
        
        self.get_logger().debug(f"Sonar 변환: 원본={sonar_distance:.2f}m, L={left_msg.range:.2f}m, F={front_msg.range:.2f}m, R={right_msg.range:.2f}m")
